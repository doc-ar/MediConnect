...
const Appointment = () => {
  ...
  const updateAppointment = async (data) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/web/update-appointment",
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  //     try {
  //         const response = await fetch('http://localhost:3001/web/update-appointment', {
  //             method: 'PATCH',
  //             headers: {
  //                 'Authorization': `Bearer ${accessToken}`,
  //                 'Content-Type': 'application/json'
  ...
  const onPopupOpen = (args) => {
    ...
    if (element && data && data.Id) {
      ...
      if (
        footerElement &&
        !footerElement.querySelector(".start-appointment-btn")
      ) {
