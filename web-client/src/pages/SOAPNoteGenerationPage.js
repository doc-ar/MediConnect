...
const SOAPNoteGenerationPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPastPrescriptions = async () => {
      try {
        const response = await fetch(`https://www.mediconnect.live/web/get-prescriptions/${patientId}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchMedicines = async () => {
      try {
        const response = await fetch("https://www.mediconnect.live/web/get-medicines", {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
  ...
  const stopRecording = async () => {
    ...
    await fetch('https://www.mediconnect.live/web/new-transcription', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`,
    ...
    if (transcript) {
      setIsLoading(true);
      try {
        const response = await fetch('https://www.mediconnect.live/web/generate-soap-notes', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`,
          },
          body: JSON.stringify({ transcript, geminiUse, patient_id: patientId }),
        });
        
        const data = await response.json();
        if (response.ok) {
          const { subjective, objective, assessment, plan } = data.soap_notes;
          console.log('SOAP notes generated:', { subjective, objective, assessment, plan });
          setSoapNotes({
            subjective,
            objective,
            assessment,
          ...
          setSoapNotes({ subjective: "Error generating SOAP notes. Please try again." });
        }
      } catch (error) {
  ...
  try {
    ...
    const getResponse = await fetch(`https://www.mediconnect.live/web/get-soapnotes/${patientId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`,
    ...
    if (!getResponse.ok) {
      console.error(`Error fetching SOAP notes: ${getResponse.status} ${getResponse.statusText}`);
    ...
    if (prevsoapNotesData && prevsoapNotesData.soap_note_id) {
      ...
      response = await fetch(`https://www.mediconnect.live/web/update-soapnote`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
      ...
      response = await fetch(`https://www.mediconnect.live/web/new-soapnote`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
  ...
  const handleAddPrescription = () => {
    ...
    setPrescription((prevPrescriptions) => [...prevPrescriptions, newPrescriptionEntry]);
    setNewPrescriptionEntry({ Dosage: '', Duration: '', Medicine: '', Frequency: '' }); // Reset form
  ...
  const formattedPrescriptionData = prescription.map((entry) => {
    const selectedMedicine = medicines.find((med) => med.label === entry.Medicine);
    if (!selectedMedicine) {
  ...
  try {
    const response = await fetch('https://www.mediconnect.live/web/new-prescription', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`,
      ...
      <div className="soap-note-page">
        <div className="top-section">
          ...
          <div className="transcription-box">
            <h3>Transcription</h3>
            <p className={transcript ? 'recording-text' : ''}>{transcript || 'Your text appears here'}</p>
          </div>
        </div>
        <label>
        <input
        ...
        <button onClick={handleSaveSOAPNotes} className="save-soap-notes-button">
          Save SOAP Notes
        </button>) : null}
        </div>
        ...
        <div className="bottom-section prescription-box">
          ...
          options={medicines}
          name="Medicine"
          value={medicines.find(option => option.label === newPrescriptionEntry.Medicine) || null} // Bind the selected value
        ...
        />
            ...
            <input
              ...
              required
            />
            <button type="button" onClick={handleAddPrescription} className="medicine-add-button">
              Add to List
            </button>
            <button type="submit" onClick={handlePrescriptionSubmit}className="prescription-submit-button">
              Submit Prescription
            </button>
          </form>
