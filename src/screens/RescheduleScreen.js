...
export default function RescheduleScreen({ route }){
    ...
    const fetchDoctorData = async () => {
        ...
        if (response.status === 200) {
            console.log("Doctors Data , Back to reschedule screen Success: ", response.data);
            const data = response.data;
            const filteredDoctor = data.filter(doctor => doctor.name === doctor_name);
    ...
    const handleSubmit =async()=>{
              console.log("Appointment Rescheduled by patient: ", appointment_id, selectedDate, selectedSlot)
                setSubmitError("");
                setSubmitMessage("")
              if(!selectedDate || !selectedSlot){
                  setSubmitError("All the Categories must be selected to Rescehdule Appointment");
                  setErrorModalVisible(true);
              }
              else{
                  const response = await FetchRequest("https://www.mediconnect.live/mobile/reschedule-appointment","patch",{
