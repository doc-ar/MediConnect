  ...
  const validateAndSubmit = async () => {
    if (Name && Contact && Address && Age && Gender) {
      ...
      if (
        ...
        setError(
          "Blood type is invalid. Use valid blood types (A+, A-, B+, etc.)",
        );
        setIsValidationModalVisible(true);
        return;
      }
      ...
      if (
        Weight &&
        (!Number.isFinite(parseFloat(Weight)) || parseFloat(Weight) <= 0)
      ) {
      ...
      if (
        Height &&
        (!Number.isFinite(parseFloat(Height)) || parseFloat(Height) <= 0)
      ) {
      ...
      if (
        BloodGlucose &&
        (!Number.isFinite(parseFloat(BloodGlucose)) ||
          parseFloat(BloodGlucose) <= 0)
      ) {
      ...
      const response = await FetchRequest(
        "http://localhost:3002/mobile/create-patient-profile",
        "post",
        {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.inputContainer}>
        ...
        <Text style={styles.required}>
          Enter your Medical data as last recorded by a medical professional. If
          you do not have the data, or if you are uncertain of the data please
          skip this part.
        </Text>
        {/* Optional Fields */}
        ...
        <View style={styles.Buttons}>
          ...
          >
            ...
            <Ionicons
              ...
              style={[styles.ButtonIcon, { paddingTop: hp(0.2) }]}
              size={hp(3)}
              color="#2F3D7E"
            />
          </TouchableOpacity>
        ...
        <Modal isVisible={isValidationModalVisible}>
          <View style={styles.ValidationModalView}>
            ...
            <MaterialIcons
              ...
              size={hp(9)}
              color="#a1020a"
              style={styles.ValidationModalIcon}
            />
            <TouchableOpacity
              onPress={() => setIsValidationModalVisible(false)}
              style={styles.ValidationModalButton}
            >
              <Text style={styles.ValidationButtonText}>Okay</Text>
            </TouchableOpacity>
          </View>
        </Modal>
