...
export default function NewAppointment() {
  ...
  const [SelectedDoctorTypeFilter, setSelectedDoctorTypeFilter] =
  ...
  useEffect(() => {
    if (SelectedDoctorTypeFilter === "designation") {
      const filteredDoctors = DoctorsList.filter(
        (doctor) => doctor.designation === DoctorType,
      );
      ...
      const filteredDoctors = DoctorsList.filter(
        (doctor) => doctor.qualification === DoctorType,
      );
      const doctors = filteredDoctors.map((doctor) => ({
        key: doctor.name,
        value: doctor.name,
  ...
  useEffect(() => {
    const selectedDoctor = DoctorsList.find(
      (doctor) => doctor.name === DoctorsName,
    );
    if (selectedDoctor) {
  ...
  useEffect(() => {
    const Slots =
      SelectedDoctorSchedule.find(
        (scheduleItem) => scheduleItem.date === selectedDate,
      )?.slots || [];
    if (Slots) {
      setSlotsList(Slots);
    }
  ...
  const fetchDoctorData = async () => {
    ...
    if (response.status === 200) {
      console.log(
        "Doctors Data , Back to Schedule screen Success: ",
        response.data,
      );
  ...
  const handleSubmit = async () => {
    console.log(
      "Appointment Scheduled by patient: ",
      DoctorType,
      DoctorsName,
      selectedDate,
      SelectedSlot,
    ...
    console.log(
      "Doctor id: ",
      DoctorsList.find((doctor) => doctor.name === DoctorsName).doctor_id,
    );

    if (!DoctorType || !DoctorsName || !selectedDate || !SelectedSlot) {
      setSubmitError(
        "All the Categories must be selected to Scehdule Appointment",
      );
      ...
      const response = await FetchRequest(
        "http://localhost:3002/mobile/create-appointment",
        "post",
        {
          status: "scheduled",
          slot_id: SelectedSlot,
          patient_id: PatientData.patient_id,
          doctor_id: DoctorsList.find((doctor) => doctor.name === DoctorsName)
            .doctor_id,
        },
      );
      if (response.status === 200) {
        ...
        showAppointmentNotification(
          `Appointment Scheduled with ${DoctorsName} on ${selectedDate} at ${selectedTime}`,
          null,
        );
      ...
      {IsDataFetched && (
        <ScrollView contentContainerStyle={styles.BottomView}>
            ...
            <View style={styles.RadioButtonTypesView}>
              <View style={styles.RadioButtonSet}>
                <RadioButton
                  value={"qualification"}
                  status={
                    SelectedDoctorTypeFilter === "qualification"
                      ? "checked"
                      : "unchecked"
                  }
                  ...
                  color="#2F3D7E"
                />
                <Text style={[styles.RadioButtonText, { marginLeft: wp(0.5) }]}>
                  qualification
                </Text>
              </View>
              <View style={styles.RadioButtonSet}>
                <RadioButton
                  value={"designation"}
                  status={
                    SelectedDoctorTypeFilter === "designation"
                      ? "checked"
                      : "unchecked"
                  }
          ...
          <SelectList
            ...
            arrowicon={
              <FontAwesome name="chevron-down" size={hp(2)} color={"black"} />
            }
                  ...
                  return (
                    <View key={index} style={styles.RadioButton}>
                      <RadioButton
                        value={slotId}
                        status={
                          slotId === SelectedSlot ? "checked" : "unchecked"
                        }
                        onPress={() => {
