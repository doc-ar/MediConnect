  ...
  useEffect(() => {
    const fetchSOAP = async () => {
      const response = await fetchRequest(
        "http://localhost:3002/mobile/get-soap-notes",
        "get",
      );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.BottomView}>
        ...
        {!Loading && Object.keys(SOAP).length === 0 && (
          <Text style={styles.LoadingText}>No SOAP Notes Data Available</Text>
        )}
        {!Loading && Object.keys(SOAP).length > 0 && SOAP.soap_note_data && (
          <View style={styles.NoteContainer}>
            ...
            <Text style={styles.ObjectText}>
              {SOAP.soap_note_data.assessment}
            </Text>

            <Text style={styles.SectionTitle}>Subjective:</Text>
            <Text style={styles.ObjectText}>
              {SOAP.soap_note_data.subjective}
            </Text>

            <Text style={styles.SectionTitle}>Objective:</Text>
            <Text style={styles.ObjectText}>
              {SOAP.soap_note_data.objective}
            </Text>
