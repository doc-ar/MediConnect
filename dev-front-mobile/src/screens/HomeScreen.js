...
export default function HomeScreen({ navigation }) {
  ...
  const fetchUpcomingAppointments = async () => {
    try {
      ...
      const response = await FetchRequest(
        "http://localhost:3002/mobile/upcoming-appointments",
        "get",
      );

      if (response.status === 200) {
        console.log(
          "Upcoming App Data, Back to Home Screen Success: ",
          response.data,
        );
        ...
        console.log(
          "Error Fetching Upcoming App Data on Home Screen: ",
          response.data,
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      const response = await FetchRequest(
        "http://localhost:3002/mobile/patient-data",
        "get",
      );
      if (response.status === 200) {
        console.log(
          "Patient Data , Back to Home Screen Success: ",
          response.data,
        );
        setLoading(false);
  ...
  useEffect(() => {
    if (ReloadAppointments) {
      console.log(
        "fetching Upcoming appointments, Reload Appointments Triggered",
      );
      setTimeout(() => {
        fetchUpcomingAppointments();
      }, 1000);
  ...
  const renderAppointmentItem = (item) => {
    return (
      <TouchableOpacity
        style={styles.LatestAppView}
        onPress={() =>
          navigation.navigate("AppointmentDetails", { AppointmentDetail: item })
        }
      >
        ...
        <View style={styles.AppointmentDetailsView}>
          ...
          <MaterialIcons
            ...
            size={hp(5)}
            color="#2F3D7E"
            style={styles.Appointment_forward_Icon}
          />
        </View>
      </TouchableOpacity>
    );
            ...
            <View style={styles.BioView2}>
              <View style={styles.row}>
                <Text style={styles.data}>
                  Blood Type: {Info.bloodtype ? Info.bloodtype : "NA"}
                </Text>
                <Text style={styles.data}>
                  Weight: {Info.weight ? Info.weight : "NA"}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.data}>
                  Blood Glucose:{" "}
                  {Info.blood_glucose ? Info.blood_glucose : "NA"}
                </Text>
                <Text style={styles.data}>
                  Height: {Info.height ? Info.height : "NA"}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.data}>
                  Blood Pressure:{" "}
                  {Info.blood_pressure ? Info.blood_pressure : "NA"}
                </Text>
                <Text style={styles.data}>
                  Age: {Info.age ? Info.age : "NA"}
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.data}>
                  Allergies: {Info.allergies ? Info.allergies : "NA"}
                </Text>
              </View>
            </View>
          </>
